...
def getIds(branch,endpoint,dateEncoded,item):
    ...
    secret = {
        ...
        "timeZone" : branch.get("timeZone", "America/Los_Angeles")
    }
    ...
    while len(results) > 0:         
        ...
        if endpoint["dateBool"] is True:
            if item == "Appointment":
                ...
                url = idUrl.format(secret["officeID"],secret["authenticationKey"],secret["authenticationToken"],dateEncoded,'&active={"operator":"=", "value":-3}')
            elif item == "Customer":
                url = idUrl.format(secret["officeID"],secret["authenticationKey"],secret["authenticationToken"],dateEncoded,urllib.parse.quote('{"operator":"IN","value":["0","1"]}'))
            elif item == "Door":
                url = idUrl.format(secret["officeID"],secret["authenticationKey"],secret["authenticationToken"],dateEncoded,urllib.parse.quote('{"operator":"IN","value":["-2","-1","0","1","2"]}'))
            ...
            url = idUrl.format(secret["officeID"],secret["authenticationKey"],secret["authenticationToken"],urllib.parse.quote('{"operator":"IN","value":["0","1"]}'))
        else:
            url = idUrl.format(secret["officeID"],secret["authenticationKey"],secret["authenticationToken"])
...
def iterateIds(ids,secret,endpoint,item):
    ...
    while length > 1000:
        ...
        # After getting the bulk data for the 1000 ID's, it gets inserted into the database.
        ...
        length = len(ids)
    # Continues the bulk loading of data after the total ID's left to request is less than 1000
    if length > 0:
...
def insertData(data,secret,endpoint,ids):
    ...
    for row in data:
        for key in row:
            if endpoint['sqlTable'] == 'PR.Subscription':
            ...
            if "key" in ('batchClosed','batchOpened','checkIn','checkOut','created','date','dateAdded','dateApplied','dateCancelled','dateChanged','dateCompleted','dateCreated','dateSent','dateSigned','dateUpdated','deleted','dueDate','emailSent','invoiceDate','lastAttemptDate','lastLogin','lastUpdated','leadDateAdded','leadDateAssigned','leadUpdated','paymentHoldDate','responseTime','sentFailureDate','timeCreated','voiceSent'):
                ...
                try:
                    ...
                    print(f"Error processing datetime field {key}: {e}")
                    print(f"Original value: {row[key]}")
                    print("Setting to default date")
